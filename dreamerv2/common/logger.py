        ...
        for name, value in dict(mapping).items():
            ...
            if len(value.shape) not in (0, 1, 2, 3, 4):
                raise ValueError(
                    ...
                    "interpreted as scalar, list[scalars], image, or video.")
...
class TensorBoardOutput:
    ...
    def __init__(self, logdir, fps=20, skipped_metrics=None):
    ...
    def __call__(self, summaries):
        ...
        for step, name, value in summaries:
            if len(value.shape) == 0:
                if not (name in self.skipped_metrics):
                    tf.summary.scalar('scalars/' + name, value, step)
            elif len(value.shape) == 1:
                if not (name in self.skipped_metrics):
                    for i in range(value.shape[0]):
                        tf.summary.scalar(f'scalars/{name}_{i}', value[i], step)
            elif len(value.shape) == 2:
                tf.summary.image(name, value, step)
            elif len(value.shape) == 3:
                tf.summary.image(name, value, step)
            elif len(value.shape) == 4:
                video_path = self._video_summary(name, value, step)
                if os.path.isfile(video_path):
                    wandb.log({f"video_{name}": wandb.Video(video_path)})
        self._writer.flush()
    ...
    def _video_summary(self, name, video, step):
        ...
        if not os.path.isdir(os.path.join(self._logdir, 'gifs')):
                os.makedirs(os.path.join(self._logdir, 'gifs'))
        ...
        try:
            ...
            clip.write_gif('{0}/{1}_{2}.gif'.format(os.path.join(self._logdir, 'gifs'), name, step), fps=10)
            # write_gif(video,
            #           '{0}/{1}_{2}.gif'.format(os.path.join(self._logdir, 'gifs'), name, step),
            #           fps=10)
        return '{0}/{1}_{2}.gif'.format(os.path.join(self._logdir, 'gifs'), name, step)
